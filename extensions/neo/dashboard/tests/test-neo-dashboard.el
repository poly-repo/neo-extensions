  ...
  (it "renders environment debug lines from framework and registry state"
    (let* ((available (make-hash-table :test 'equal))
      ...
      (cl-letf (((symbol-function 'neo/get-emacs-instance-name)
                ...
                ((symbol-function 'dashboard-init--info)
                 (lambda () "1 packages installed. Emacs started in 0.5 seconds.")))
        (let ((neo--framework framework)
              (neo/cache-directory "/tmp/neo-cache"))
          (setq lines (neo/dashboard--debug-lines))))
      (expect lines :to-contain (cons "Instance: neo-test" nil))
      (expect lines :to-contain (cons "Startup: 1 packages installed. Emacs started in 0.5 seconds." nil))
      (expect lines :to-contain (cons "Extension source: local checkout" nil))
      (expect lines :to-contain (cons "Cache dir: /tmp/neo-cache" nil))
      (expect lines :to-contain (cons "Data dir: /tmp/neo-data" nil))
      (expect lines :to-contain (cons "Enabled roots: (\"neo:full-monty\")" nil))
      (expect lines :to-contain (cons "neo:full-monty available=yes installed=yes" nil))
      (expect lines :to-contain (cons "neo:haskell available=yes installed=no" nil))
      (expect lines :to-contain (cons "neo: local /tmp/neo-local" nil))
      (expect lines :to-contain (cons "mav: cached /cache/extensions/mav/extensions-current.el [missing]" nil))
      (expect lines :to-contain (cons "Package conflicts: 0" nil))))

  (it "highlights a non-zero package conflict count with a RET hint"
    (cl-letf (((symbol-function 'neo/package-conflict-count) (lambda () 2))
              ((symbol-function 'neo/package-conflicts-summary) (lambda () nil))
              ((symbol-function 'neo/get-emacs-instance-name) (lambda () "neo-test")))
      (let* ((lines (neo/dashboard--debug-lines))
             (entry (assoc "Package conflicts: 2 (RET for details)" lines)))
        (expect entry :to-be-truthy)
        (expect (cdr entry) :to-equal #'neo/package-conflicts-summary)))))
