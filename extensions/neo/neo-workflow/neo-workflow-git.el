...
;; Verbatim copy from workflow/neo-workflow-git.el.
;; No GitHub or SQLite dependencies.
...
(defun neo--workflow-git-commits-in-feature-branch (branch parent)
  "Get commits in feature BRANCH relative to PARENT."
...
(defun neo--workflow-git-commits-in-perennial-branch ()
  "Get recent commits in the current perennial branch."
...
(defun neo--workflow-git-parse-commits (output)
  "Parse OUTPUT into a list of (sha . subject) pairs."
  (mapcar (lambda (line)
...
(defun neo/workflow-git-create-and-checkout-branch-with-parent (name parent)
  ...
  (let ((args (list "checkout" "-b" name parent)))
    (when (string-match-p "^origin/" parent)
...
(defun neo/workflow-git-fetch-upstream (branch)
  ...
  (neo--workflow-git-run "fetch" "upstream" branch))
...
(defun neo/workflow-git-push-local-branch (sha branch remote push-hook)
  "Push local branch SHA as BRANCH to REMOTE."
  (let ((args (list "push")))
    (unless push-hook (setq args (append args '("--no-verify"))))
...
(defun neo/workflow-git-rebase-onto (onto remove)
  "Rebase onto ONTO, removing REMOVE."
  ...
  (let ((statuses nil)
    ...
    (while (< i len)
      (let* ((xy (substring output i (+ i 2)))
        ...
        (push xy statuses)
        (if path-end
            (let ((_path (substring output path-start path-end)))
              (when (or (eq (aref xy 0) ?R) (eq (aref xy 1) ?R))
                (let ((new-path-end (string-match "\0" output (1+ path-end))))
                  (when new-path-end
...
(defun neo/workflow-git-worktree-list ()
  "Return a list of plists describing current worktrees.
Each plist has :path, :branch, :head, :bare."
  (let ((output (ignore-errors
                  (neo--workflow-git-query "worktree" "list" "--porcelain"))))
    (when output
      (let ((result nil)
            (current nil))
        (dolist (line (split-string output "\n"))
          (cond
           ...
           ((string-prefix-p "HEAD " line)
            (setq current (plist-put current :head (string-remove-prefix "HEAD " line))))
...
(defun neo/workflow-git-worktree-path-for-branch (branch-name)
  "Return the worktree path for BRANCH-NAME, or nil if not checked out."
  (when-let* ((worktrees (neo/workflow-git-worktree-list))
              (entry (seq-find (lambda (wt)
                                 (string= (plist-get wt :branch) branch-name))
                               worktrees)))
    ...
    (mapcar (lambda (line)
              (let ((parts (split-string line " ")))
                (list :refname (string-remove-prefix "refname:" (nth 0 parts))
                      ...
                      :track (string-join (nthcdr 7 parts) " "))))
            (neo--workflow-git-lines output))))

(defun neo--workflow-git-determine-sync-status (track upstream)
  "Determine sync status symbol from TRACK string and UPSTREAM name."
...
(defun neo/workflow-git-merge-conflicts (file-conflicts _parent-location _root-branch)
  "Return FILE-CONFLICTS enriched with blob info (stub — enrichment deferred)."
  file-conflicts)
...
(defun neo/workflow-git-resolve-conflict (file resolution)
  ...
  (let ((flag (pcase resolution
                (:ours "--ours")
                (:theirs "--theirs")
                (_ (error "Unknown resolution: %s" resolution)))))
