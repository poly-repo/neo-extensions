...
;; Verbatim copy from workflow/neo-workflow-slug.el.
...
(defun neo-issue-slug--stopword-p (token)
  "Return non-nil if TOKEN is a stopword."
  (member token neo-issue-slug-stopwords))

(defun neo-issue-slug--replace (token)
  "Return replacement for TOKEN, or TOKEN itself if no replacement exists."
  (or (cdr (assoc token neo-issue-slug-replacements))
      token))

(defun neo-issue-slug--filter-tokens (tokens)
  "Remove stopwords from TOKENS unless they are preserved."
  (seq-filter
   (lambda (tok)
...
(defun neo-issue-slug--drop-tail (tokens)
  "Drop unwanted trailing words from TOKENS."
  (while (and tokens
...
;;; neo-workflow-slug.el ends here
